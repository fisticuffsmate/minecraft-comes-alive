    ...
    public void receive(ServerPlayerEntity player) {
        ...
        if (Config.getInstance().allowDestinyTeleportation && location != null) {
            WorldUtils.getClosestStructurePosition(player.getServerWorld(), player.getBlockPos(), new Identifier(location), 128).ifPresent(pos -> {
                ...
                player.getServerWorld().getChunkManager().addTicket(ChunkTicketType.POST_TELEPORT, chunkPos, 1, player.getEntityId());
                player.networkHandler.requestTeleport(pos.getX(), pos.getY(), pos.getZ(), player.yaw, player.pitch, EnumSet.noneOf(PlayerPositionLookS2CPacket.Flag.class));
