...
public class DestinyScreen extends VillagerEditorScreen {
    ...
    protected void setPage(String page) {
        if (page.equals("destiny") && !allowTeleportation) {
            ...
            if (Config.getInstance().oldDestinyLocations.size() == 1) {
                selectStory(Config.getInstance().oldDestinyLocations.get(0));
                return;
            }
        }
        ...
        switch (page) {
            ...
            case "destiny":
                int x = 0;
                int y = 0;
                for (String location : Config.getInstance().oldDestinyLocations) {
                    int rows = (int)Math.ceil(Config.getInstance().oldDestinyLocations.size() / 3.0f);
                    float offsetX = (y + 1) == rows ? (2 - (Config.getInstance().oldDestinyLocations.size() - 1) % 3) / 2.0f : 0;
