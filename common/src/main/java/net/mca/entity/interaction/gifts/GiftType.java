...
public class GiftType {
    ...
    public static GiftType fromJson(Identifier id, JsonObject json) {
        ...
        HashMap<Tag.Identified<Item>, Integer> tags = new HashMap<>();
        JsonHelper.getObject(json, "items").entrySet().forEach(element -> {
            ...
            if (string.charAt(0) == '#') {
                ...
                Tag.Identified<Item> tag = TagHooks.getItemOptional(identifier);
                if (tag != null) {
    ...
    private final Map<Tag.Identified<Item>, Integer> tags;
    ...
    public GiftType(Identifier id, int priority, List<GiftPredicate> conditions, Map<Item, Integer> items, Map<Tag.Identified<Item>, Integer> tags, int fail, int good, int better, Map<Response, String> responses) {
        this.id = id;
    ...
    public boolean matches(ItemStack stack) {
        return items.keySet().stream().anyMatch(i -> i == stack.getItem()) || tags.keySet().stream().anyMatch(stack.getItem()::isIn);
    }
    ...
    public IntAnalysis getSatisfactionFor(VillagerEntityMCA recipient, ItemStack stack, ServerPlayerEntity player) {
        ...
        int base = value.orElseGet(() -> tags.entrySet().stream().filter(i -> stack.getItem().isIn(i.getKey())).findFirst().map(Map.Entry::getValue).orElse(0));
